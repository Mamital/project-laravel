                    ...
                    <section class="content-wrapper bg-white p-3 rounded-2 mb-2">
                        ...
                        @forelse ($products as $product)
                            ...
                            <section class="align-self-start w-100">
                                ...
                                <p><i class="fa fa-store-alt cart-product-selected-store me-1"></i> <span>{{$product->marketable_number > 0 ? 'موجود در انبار' : 'کالا موجود نیست' }}</span></p>
                                <section>
                                    <a class="text-decoration-none cart-delete" href="{{route('home.profile.my-favorite.delete', $product)}}"><i class="fa fa-trash-alt"></i> حذف از لیست علاقه ها</a>
                                </section>
                            </section>
                            <section class="align-self-end flex-shrink-1">
                                            @if (!empty($product->activeAmazingSales()))
                                                <section class="cart-item-discount text-danger text-nowrap mb-1">
                                                    {{ priceFormat($product->price * ($product->activeAmazingSales()->percentage / 100)) }} تخفیف
                                                </section>
                                            @endif
                                            <section class="text-nowrap fw-bold">
                                                {{ priceFormat($product->price - $product->price * ($product->activeAmazingSales()->percentage / 100)) }} تومان</section>
                                        </section>
                        </section>
