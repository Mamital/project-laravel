...
@section('content')
@if ($products->count() > 0)
    <section class="lazyload-wrapper" style="margin-right: 5rem">
                    ...
                    <section class="lazyload-item-wrapper">
                        <section class="product">
                            <section class="product-add-to-cart"><a
                                    href="{{ route('home.profile.my-compare.remove', $product) }}" data-bs-toggle="tooltip"
                                    data-bs-placement="left" title="حذف از مقایسه"><i class="fa fa-trash"></i></a>
                            </section>
                ...
                <td style="text-align: center">
                    @foreach ($property->values as $value)
                        @if ($value->product_id == $product->id)
                            {{ json_decode($value->value)->value }} {{ $value->attribute->unit }}
                        @endif
                    @endforeach
                </td>
            @endforeach
