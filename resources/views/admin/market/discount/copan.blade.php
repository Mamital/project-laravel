                        ...
                        <tbody>
                            @foreach ($copans as $copan)
                                <tr>
                                    ...
                                    <th>{{ $copan->code }}</th>
                                    <th>{{ $copan->amount_type == 0 ? $copan->amount . '%' : $copan->amount}}</th>
                                    <th>{{ $copan->amount_type == 0 ? 'درصدی' : 'عددی' }}</th>
                                    <th>{{ $copan->discount_ceiling ? $copan->discount_ceiling : '-' }}</th>
                                    <th>{{ $copan->type == 0 ? 'عمومی' : 'خصوصی' }}</th>
                                    <td>{{ persian(jalalidate($copan->start_date)) }}</td>
                                    <td>{{ persian(jalalidate($copan->end_date)) }}</td>
                                    <td class="width-16-rem text-left">
                                        <a href="{{route('admin.market.discount.copan.edit', $copan->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i>
                                            ویرایش</a>
                                        <form class="d-inline"
